...
In addition to setting the environment variables this hubot also supports settings via ini file.

The ini file `hubot_secrets.ini` must be in the folder of myhubot
with the section `environment` otherwise hubot will trow and error.
...
There is also a possibility to set the environment variables with the INI file called
`hubot_secrets.ini`. Example:
...
This will populate the environment variables with:

    ENVIRONMENT_FOO=bar
    FOO=bar
    HUBOT_IRC_ROOMS=#baz

The name of the shell variable is composed from the INI section (changed to upper case) and the variable name.
Then the value is added. Try to omit quotation marks.
